...
graph.nodes.forEach(node => 
{
  ...
  if(!nodeObject.hasOwnProperty("name")) 
  { 
    let stdNames = ["C","C#","D","Eb","E","F","F#","G","Ab","A","Bb","B"];
    let flatNames = ["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"];
    let sharpNames = ["C","Db","D","Eb","E","F","Gb","G","Ab","A","Bb","B"];
    
    // If the object is a note
    if(nodeObject.id < 12)
    {
      nodeObject.name      = stdNames[nodeObject.id]; 
      nodeObject.nameSharp = flatNames[nodeObject.id]; 
      nodeObject.nameFlat  = sharpNames[nodeObject.id]; 
    }
    else
    {
      var majorminor = nodeObject.voicing === "minor" ? "m" : "";
      nodeObject.name      = stdNames[nodeObject.root]   + majorminor; 
      nodeObject.nameSharp = flatNames[nodeObject.root]  + majorminor;
      nodeObject.nameFlat  = sharpNames[nodeObject.root] + majorminor; 
    }
  }
